    ...
    with torch.no_grad():
        ...
        with torch.no_grad():
            ...
            for i,profile in enumerate(plotted_profiles):
                axes[i,1].errorbar(plotted_times, predicted_means[profile][:,rho_ind], yerr=predicted_stds[profile][:,rho_ind],
                                   label='predicted', c='k', alpha=0.1)
                axes[i,1].plot(plotted_times, denormalized_true_dic[profile][:,rho_ind],
                               label='real', c='k', linestyle='--')
                axes[i,1].set_ylabel(label_map[profile])
                for which_color, time_ind in enumerate(plotted_time_inds):
                    axes[i,2].plot(x, predicted_means[profile][time_ind], c=colors[which_color],
                                   label=f'{plotted_times[time_ind]}ms')
                    axes[i,2].plot(x, denormalized_true_dic[profile][time_ind],
                                   linestyle='--', c=colors[which_color])
            for i,actuator in enumerate(plotted_actuators):
                axes[i,0].plot(plotted_times, denormalized_true_dic[actuator][0],
                               label='real', c='k', linestyle='--')
                axes[i,0].set_ylabel(label_map[actuator])
            if len(plotted_parameters)>0:
                for i,parameter in enumerate(plotted_parameters):
                    axes[i,3].errorbar(plotted_times, predicted_means[parameter][:, 0], yerr=predicted_stds[parameter][:, 0],
                                   label='predicted', c='k', alpha=0.1)
                    axes[i,3].plot(plotted_times, denormalized_true_dic[parameter],
                                   label='real', c='k', linestyle='--')
