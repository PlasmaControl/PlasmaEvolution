...
with torch.no_grad():
    ...
    for i,profile in enumerate(plotted_profiles):
        axes[i,1].errorbar(times, predicted_means[profile][:,rho_ind], yerr=predicted_stds[profile][:,rho_ind],
                           label='predicted', c='k', alpha=0.1)
        axes[i,1].plot(times, denormalized_true_dic[profile][:,rho_ind],
    ...
    for i,actuator in enumerate(plotted_actuators):
        axes[i,0].plot(times, denormalized_true_dic[actuator][0],
                       label='real', c='k', linestyle='--')
...
fig.suptitle(f'Shot {shot} fake {label_map[fake_actuator_name]}')
plt.savefig(f'plots/{output_filename_base}_plots{shots[sample_ind]}-fake{fake_actuator_name}.svg', format='svg')
plt.show()
